...
bool YuBoudingbox::isInside(float _x, float _y) {
	if(_x >= x1 && _x <= x1 + width && _y <= y1 && _y >= y1 - height) {
		return true;
	}
	return false;
}
bool YuBoudingbox::isInsideRight(float _x, float _y) {

	if(_y <= y1 && _y >= y1 - height && _x >= x1 + width / 2 && _x <= x1 + width) {
		return true;
	}
	return false;

}
bool YuBoudingbox::isInsideLeft(float _x, float _y) {

	if(_y <= y1 && _y >= y1 - height && _x >= x1 && _x <= x1 + width / 2) {
		return true;
	}
	return false;
